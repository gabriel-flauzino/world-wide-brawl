...
export class Matrix2x3 extends Matrix {
  ...
  static fromMatrix(m) {
    return new Matrix2x3(m?.a, m?.b, m?.c, m?.d, m?.tx, m?.ty);
  }
  ...
  initFromFlatBufferShort(matrix) {
    ...
    this.tx = matrix.x() / Matrix2x3.TWIP_MULTIPLIER;
    this.ty = matrix.y() / Matrix2x3.TWIP_MULTIPLIER;
  }
  ...
  multiply(m) {
    ...
    const x = m.applyX(this.tx, this.ty);
    const y = m.applyY(this.tx, this.ty);

    this.a = a; this.b = b; this.c = c; this.d = d; this.tx = x; this.ty = y;
  }
  ...
  applyY(x, y) {
    return y * this.d + x * this.b + this.ty;
  }
}

// -------------- DEPRECATED --------------

export class Matrix2x3Old {
  static PRECISE_MULTIPLIER = 65535;
  static DEFAULT_MULTIPLIER = 1024;
  static PRECISE_FLOAT = 0.0009765;
  static TWIP_MULTIPLIER = 20;
